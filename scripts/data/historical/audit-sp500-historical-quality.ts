...
async function main(): Promise<void> {
  ...
  const [missingSource, invalidRows, provenance] = await Promise.all([
    ...
    prisma.$queryRawUnsafe<Array<{ total_rows: number; yahoo_rows: number; non_yahoo_rows: number }>>("SELECT COUNT(*)::int AS total_rows, COUNT(*) FILTER (WHERE source = 'YAHOO')::int AS yahoo_rows, COUNT(*) FILTER (WHERE source IS DISTINCT FROM 'YAHOO')::int AS non_yahoo_rows FROM stock_history WHERE stock_id = ANY($1::text[])", ids),
  ]);
  console.log(JSON.stringify({ missingYahooSource: missingSource, invalidOhlcvRows: invalidRows, provenance: provenance[0] }, null, 2));
}
