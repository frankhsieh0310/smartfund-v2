...
type Quarter = { start: string; end: string; value: number; form: string; filed: string; accepted: string; accession: string; fiscalYear?: number };
...
function newer(a: SecUnit, b: SecUnit) { return String(a.filed ?? "") > String(b.filed ?? "") ? a : b; }
function standaloneQuarters(items: SecUnit[], acceptance: Map<string, string>): Quarter[] {
  const latest = new Map<string, SecUnit>();
  for (const item of items.filter((item) => item.start && item.end && typeof item.val === "number" && ["10-Q", "10-K"].includes(item.form ?? ""))) { const key = `${item.start}:${item.end}`; latest.set(key, latest.has(key) ? newer(latest.get(key)!, item) : item); }
  ...
  for (const item of base) {
    ...
    if (days >= 70 && days <= 115) result.push({ start: item.start, end: item.end, value: item.val, form: item.form!, filed: item.filed!, accepted, accession: item.accn ?? "", fiscalYear: item.fy });
  }
  for (const item of base) {
    ...
    const contained = result.filter((quarter) => quarter.start >= item.start! && quarter.end <= item.end!).sort((a, b) => a.end.localeCompare(b.end));
    if (contained.length !== 3 || result.some((quarter) => quarter.end === item.end)) continue;
    const value = item.val - contained.reduce((sum, quarter) => sum + quarter.value, 0);
    if (!Number.isFinite(value)) continue;
    const start = new Date(`${contained.at(-1)!.end}T00:00:00Z`); start.setUTCDate(start.getUTCDate() + 1);
    result.push({ start: start.toISOString().slice(0, 10), end: item.end, value, form: item.form!, filed: item.filed!, accepted, accession: item.accn ?? "", fiscalYear: item.fy });
  }
  return result.sort((a, b) => a.end.localeCompare(b.end));
}
function ttmSeries(quarters: Quarter[]): Ttm[] { return quarters.map((quarter, index) => { const prior = quarters.slice(Math.max(0, index - 3), index + 1); return prior.length === 4 ? { ...quarter, ttm: prior.reduce((sum, item) => sum + item.value, 0) } : null; }).filter((item): item is Ttm => Boolean(item)); }
...
async function main() {
  ...
  for (const stock of stocks) {
    ...
    const quarters = standaloneQuarters(eps, accepted); const ttms = ttmSeries(quarters);
    const epsRows = quarters.map((item) => ({ stockId: stock.id, metric: "eps.diluted.quarter", periodStart: date(item.start), periodEnd: date(item.end), fiscalPeriod: "QUARTER", formType: item.form, filingDate: date(item.filed), publicationDate: date(item.accepted), value: item.value, unit: "USD_PER_SHARE", currency: "USD", source: "SEC_EDGAR_COMPANYFACTS", sourceFactKey: `${id}:eps-quarter:${item.end}:${item.accession}`, sourceDocumentUrl: `https://data.sec.gov/api/xbrl/companyfacts/CIK${id}.json`, restatementVersion: item.filed }));
    const ttmRows = ttms.map((item) => ({ stockId: stock.id, metric: "eps.diluted.ttm", periodEnd: date(item.end), fiscalPeriod: "TTM", formType: item.form, filingDate: date(item.filed), publicationDate: date(item.accepted), value: item.ttm, unit: "USD_PER_SHARE", currency: "USD", source: "SEC_EDGAR_DERIVED", sourceFactKey: `${id}:eps-ttm:${item.end}:${item.accepted}`, sourceDocumentUrl: `https://data.sec.gov/api/xbrl/companyfacts/CIK${id}.json`, restatementVersion: item.filed }));
    const peRows = prices.flatMap((price) => { const available = ttms.filter((item) => availabilityDate(item.accepted) <= price.date).at(-1); const close = restorePreSplitClose(Number(price.close), price.date, splits); return available && available.ttm > 0 && Number.isFinite(close) ? [{ stockId: stock.id, metric: "valuation.pe.ttm.point_in_time", periodEnd: price.date, fiscalPeriod: "DAILY", publicationDate: date(available.accepted), value: close / available.ttm, unit: "RATIO", currency: "USD", source: "SEC_EDGAR_PLUS_YAHOO_PRICE", sourceFactKey: `${id}:pe:${price.date.toISOString().slice(0, 10)}:${available.accepted}`, sourceDocumentUrl: "SEC companyfacts + Yahoo split events + existing Yahoo price history" }] : []; });
    await prisma.stockFinancialFact.deleteMany({ where: { stockId: stock.id, source: { in: ["SEC_EDGAR_DERIVED", "SEC_EDGAR_PLUS_YAHOO_PRICE"] }, metric: { in: ["eps.diluted.ttm", "valuation.pe.ttm.point_in_time"] } } });
