...
export async function loadLifecycleResumeCheckpoint(prisma: PrismaClient, jobId: string): Promise<ResumeCheckpoint | null> {
  const rows = await prisma.$queryRawUnsafe<Array<Omit<ResumeCheckpoint, "details"> & { details: unknown }>>(
    // An orphaned run is intentionally marked ABANDONED before its lock is
    // released. Its checkpoint is still the authoritative continuation point.
    "SELECT c.last_symbol, c.processed, c.succeeded, c.failed, r.details FROM production_scheduler_checkpoints c JOIN production_scheduler_runs r ON r.id = c.run_id WHERE c.job_id = $1 AND r.status IN ('IN_PROGRESS', 'PAUSED', 'ABANDONED') ORDER BY c.updated_at DESC LIMIT 1",
    jobId,
  );
  const row = rows[0];
