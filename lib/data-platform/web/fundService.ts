...
// Production base (fa937d7) has no `domicile` column on Fund yet — this candidate targets that
// schema as-is (no schema changes allowed here), so identity falls back to `region` for both.
const fundIdentity = (row: { id: string; code: string | null; isin: string | null; name: string; currency: string; region: string | null }): CanonicalIdentity => ({ assetType: "FUND", id: row.id, symbol: row.code ?? row.isin ?? row.id, name: row.name, displayName: row.name, currency: row.currency, market: row.region, country: row.region });
